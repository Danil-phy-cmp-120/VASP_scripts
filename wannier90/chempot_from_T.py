...
dos_up_interp = interpolate.interp1d(dos_up[:,0], dos_up[:,1]) #, fill_value="extrapolate")

#print(dos_up_interp)

dos_down_interp = interpolate.interp1d(dos_down[:,0], dos_down[:,1]) #, fill_value="extrapolate")

#energy_interp = np.linspace(min(dos_up[:,0]), max(dos_up[:,0]), 1000)
energy_interp = np.linspace(-5, max(dos_up[:,0]), 1000)
#print(energy_interp)


#dos = np.linspace(( energy_interp, (dos_up_interp(energy_interp) + dos_down_interp(energy_interp))/2 ))
...
np.savetxt('mu_T.dat', mu_T, fmt='%.1f\t%.18e')

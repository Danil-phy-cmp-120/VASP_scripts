...
for ri, r in enumerate(all_prod.keys()):
    points_for_convex += [[barycentric_to_cartesian(coords[ri,0], coords[ri,1], coords[ri,2])[0], barycentric_to_cartesian(coords[ri,0], coords[ri,1], coords[ri,2])[1], all_prod[r]]]
points_for_convex = np.array(points_for_convex)

hull = ConvexHull(points_for_convex)

data_convex = np.array([points_for_convex[i] for i in hull.vertices])
f_convex = interpolate.interp2d(data_convex[:,0], data_convex[:,1], data_convex[:,2], kind='linear')


energy_above_hull = []
for ri, r in enumerate(all_prod.keys()):
    energy_above_hull += [f_convex(barycentric_to_cartesian(coords[ri,0], coords[ri,1], coords[ri,2])[0], barycentric_to_cartesian(coords[ri,0], coords[ri,1], coords[ri,2])[1]) - all_prod[r]]
energy_above_hull = np.array(energy_above_hull)

for ri, r in enumerate(all_prod.keys()):
   print(r, energy_above_hull[ri])
...
ax.text(*rpos, 'Ga', color='C2', rotation=  0, **kwargs_label, size = 16)

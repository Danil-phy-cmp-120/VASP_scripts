...
if not os.path.isdir('orbital_resolved_DOS'):
    os.makedirs('orbital_resolved_DOS')
...
for m in range(len(lattice)):
    for i in range(len(z_atoms)):
        ...
        if xml.converged:
            ...
            for s in sites:
                #energies = xml.complete_dos.get_site_t2g_eg_resolved_dos(xml.structures[-1][s]).energies - xml.complete_dos.get_site_dos(xml.structures[-1][s]).efermi
                #dos_up = xml.complete_dos.get_site_t2g_eg_resolved_dos(xml.structures[-1][s]).get_densities(Spin(1))
                #dos_down = -xml.complete_dos.get_site_t2g_eg_resolved_dos(xml.structures[-1][s]).get_densities(Spin(-1))
                #print(xml.complete_dos.get_site_t2g_eg_resolved_dos(xml.structures[-1][s])["t2g"])
            ...
            fig.savefig('orbital_resolved_DOS/DOS_{}_{}.png'.format(lattice[m], z_atoms[i]), transparent = False, bbox_inches = 'tight')

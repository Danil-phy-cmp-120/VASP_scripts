...
for i in range(len(z_atoms)):
    ...
    if not os.path.isdir('band/inverse/step2/{}'.format(z_atoms[i])):
        os.makedirs('band/inverse/step2/{}'.format(z_atoms[i]))

    os.link('band/inverse/step1/{}/CHGCAR'.format(z_atoms[i]), 'band/inverse/step2/{}/CHGCAR'.format(z_atoms[i]))

    cell = Structure.from_file('band/inverse/step1/{}/POSCAR'.format(z_atoms[i]))
    ...
    VaspInput.write_input('band/inverse/step2/{}'.format(z_atoms[i]))
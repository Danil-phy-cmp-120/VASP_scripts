        ...
        for spin in [Spin.up, Spin.down]: 
            ...
            for k in range(1, len(bands.kpoints)):
                ...
                for b in range(bands.nb_bands):
                    ...
                    s = bands.projections[spin][b, k, 0, site]
                    p = bands.projections[spin][b, k, 1, site] + bands.projections[spin][b, k, 2, site] + bands.projections[spin][b, k, 3, site]
                    d = bands.projections[spin][b, k, 4, site] + bands.projections[spin][b, k, 5, site] + bands.projections[spin][b, k, 6, site] + bands.projections[spin][b, k, 7, site] + bands.projections[spin][b, k, 8, site]
        
                    contrib += [[kpoints_coords, bands.bands[spin][b, k] - bands.efermi, s, p, d]]   # 0:s 1:py 2:pz 3:px 4:dxy 5:dyz 6:dz2 7:dxz 8:dx2_y2 
            ...
            save_scatter(sc, 's_{}_{}'.format(spin, bands.structure[site].species), kpoints_ticks_cut, kpoints_label_cut)

            sc = plt.scatter(contrib[:,0], contrib[:,1], c = contrib[:,3] , cmap=truncate_colormap(plt.get_cmap('hot'), maxval=0.8), s = 5)
            save_scatter(sc, 'p_{}_{}'.format(spin, bands.structure[site].species), kpoints_ticks_cut, kpoints_label_cut)
            
            sc = plt.scatter(contrib[:,0], contrib[:,1], c = contrib[:,4] , cmap=truncate_colormap(plt.get_cmap('hot'), maxval=0.8), s = 5)
            save_scatter(sc, 'd_{}_{}'.format(spin, bands.structure[site].species), kpoints_ticks_cut, kpoints_label_cut)
